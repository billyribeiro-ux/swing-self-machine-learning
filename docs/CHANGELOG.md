...
- Added Prospective Time-Exit Diagnostic Ledger V1 for Sector Rotation BUY ORDINARY, with append-only diagnostic run/events tables, strict no-backfill blocking, next-open diagnostic entry, 20-session time-exit maturation, baseline-versus-experimental policy touch outcomes, CLI init/update/status/export commands, Signal Board/Candidate Detail/Reports dashboard visibility, generated CSV exports, and tests for lifecycle idempotency, label separation, no live actionability, no final-holdout promotion path, no FMP calls, and operational immutability coverage.
- Added Time-Exit Utility Label V1 for Sector Rotation BUY ORDINARY, with versioned label-side `sector_rotation_buy_ordinary_time_exit_utility_v1` outcomes, baseline and experimental policy association, calibration-only and development-holdout diagnostic summaries, a research-only time-exit hypothesis, separate signal-score components, dashboard diagnostics, CSV/XLSX exports, and tests proving label separation, next-open/horizon-close timing, costs, feature guards, no FMP calls, no model-artifact mutation, and operational immutability coverage.
- Added an experimental Sector Rotation BUY ORDINARY target/stop policy candidate for Multi-Angle Signal Discovery, with a typed `target_stop_policy_candidate_v1` registry, calibration-only `T2.0/S1.25/20D` selection evidence, separate derived candidate-policy outcomes, policy-aware hypothesis/gate/candidate artifacts, dashboard diagnostics, CSV/JSON exports, and tests proving the default policy remains unchanged, labels stay separate from features, and no FMP/model-artifact/SQLite mutation occurs in tests.
- Added Multi-Angle Calibration Audit Artifacts V1 for Signal Discovery, persisting calibration-only raw/calibrated TBS probability distributions, diagnostic threshold tables, probability buckets, row-level calibration audits, explicit insufficient-evidence rows, generation metadata hashes, Candidate Detail diagnostics, Reports/Exports downloads, and tests for split isolation, diagnostic-only thresholds, no FMP calls, no SQLite mutation, no model-artifact mutation, and operational immutability.
