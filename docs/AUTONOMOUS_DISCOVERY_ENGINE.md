...
9. Fit head-specific train-only feature screens where heads use different targets. The target-before-stop classifier screens against `label_{direction}_target_before_stop_{horizon}`. Expected-return screens against signed directional return divided by close-known signal-date `atr_pct_14`; MFE screens against favorable magnitude divided by `atr_pct_14`; MAE screens against adverse magnitude divided by `atr_pct_14`.
10. Train active path heads in ATR units under `atr_normalized_path_targets_v1`. MFE and MAE still use domain-preserving path-magnitude estimators under `path_metric_magnitude_domain_v1`, and logistic-family MFE/MAE heads remain retired.
11. Convert path predictions back into canonical decimal-return units before scanner output, selection policy, attribution, portfolio replay, and paper-forward testing. OOD Governance V2 evaluates active path heads in normalized model space and separately validates canonical MFE/MAE sign output.
