...
Under ATR-normalized path-target modeling, OOD governance evaluates active path models in normalized model space. Expected return, favorable MFE magnitude, and adverse MAE magnitude are compared against their own ATR-unit training-target bounds. The scanner separately validates canonical decimal-return outputs, including MFE `>= 0` and MAE `<= 0`, before any row can become actionable.
