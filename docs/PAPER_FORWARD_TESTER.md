...
Sample sufficiency is governed by the frozen `prospective_final_holdout_sample_v1` policy stored on each run. `final-holdout-status` reports per-model progress for matured outcomes, distinct signal dates, observation sessions, calendar months, positive/negative target-before-stop outcomes, pending entries, open positions, provenance failures, blocked backfills, data-integrity events, artifact integrity, and promotion eligibility. `final-holdout-evaluate --run-id <RUN_ID>` refuses evaluation until sample sufficiency is complete; `--diagnostic-only` allows non-promotable early diagnostics only after 30 matured outcomes and 20 signal dates.
