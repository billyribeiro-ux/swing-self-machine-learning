...
Path-metric regression screens use the same eligibility, train-only missingness/variance/imputation, deterministic score ordering, and correlation-pruning contract, but score surviving features with `mutual_info_regression` against the head's exact training representation. Expected return screens against the signed directional return target. MFE screens against favorable magnitude, equal to the existing MFE label. MAE screens against adverse magnitude, equal to negative existing MAE. Feature sets may overlap naturally, but none may inherit the primary classifier's feature list for convenience.
